...
uint8_t guestMacAddress[6] = {0xec, 0x81, 0x93, 0x0d, 0xf6, 0xe9};

// Wifi settings
const char* ssid = "BT-GFZ6";                 // Set name of Wifi Network
const char* password = "C9Xdpg9rVFAq";
...
//
//MQTT_sub_topic[1] = "run"; // pub/sub topics
//MQTT_pub_topic[1] = "run"; // You might want to create your own
//
//MQTT_sub_topic[2] = "broken"; // pub/sub topics
//MQTT_pub_topic[2] = "broken"; // You might want to create your own
...
// delay to fix strange bug with buttonA sporadically firing
// delay fix found in online forum 
delay(10);

if (M5.BtnA.wasPressed()){
   publishMessage(0, "Supplies ");
}

if (M5.BtnB.wasPressed()){
   publishMessage(1, "Run Bath ");
}

if (M5.BtnC.wasPressed()){
   publishMessage(2, "Report Broken ");
}

  // Just incase we print so much text we run off the
  // screen!  Clear screen, set cursor back to the top.
  if( M5.Lcd.getCursorY() > M5.Lcd.height() - 40 ) {
      setupScreen();
...
// pass that.
void publishMessage(int n,  String message ) {
...
//      M5.Lcd.println( message );
      ...
      // Send
      ps_client.publish( MQTT_pub_topic[n], msg );
    }
  ...
  while (!ps_client.connected()) {
    ...
    for(i=0; i < 4; i++){
      if (ps_client.connect( id_array ) ) {
        ...
        // ... and resubscribe
        ps_client.subscribe( MQTT_sub_topic[i] );
      } else {
