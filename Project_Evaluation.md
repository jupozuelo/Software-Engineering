...
In the future, the team is looking to implement a Lab Usability Testing method for all systems since the user’ s knowledge and perception is different from the developers’ ones. Test subjects will be asked to complete specific tasks on the Web App, Desktop App and the M5Stack. Although it is not certainly reflective of real-life usage of Web App and customer remote experience considering the tests take place in a controlled environment, it is still beneficial because it allows Key Stakeholders to take part as observers of the process for a better understanding of the customers behaviour and will assist in solving the issues that would bring an unpleasant user experience . As the Bubble products will become more sophisticated, the team will run A/B testing for optimization which will add value to the user interface and navigation of both the Desktop App and Web App. This implies the test subjects experimenting with two or more versions of the page in order to assess which is considered to be the most adequate.
